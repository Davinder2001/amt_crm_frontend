...
interface FetchEmployee {
  ...
  number: string;
  company_name: string;
  salary?: number | string; // Added salary field
}
...
const SalaryView: React.FC = () => {
  ...
  const employees: FetchEmployee[] = employeesData?.employees.map((emp) => ({
    ...
    salary: emp.salary ?? 'N/A', // Ensure salary is provided or fallback to 'N/A'
  })) ?? [];
      ...
      <table style={{ border: '1px solid black', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            ...
            <th style={{ border: '1px solid black', padding: '4px' }}>Salary</th> {/* Added Salary header */}
        ...
        <tbody>
          {employees.map((employee, index) => (
            <tr key={employee.id}>
              ...
              <td style={{ border: '1px solid black', padding: '4px' }}>{employee.salary}</td> {/* Salary column */}
