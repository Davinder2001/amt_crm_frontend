...
function ChatPage() {
  ...
  // Unique key to force message component to re-render
  const chatKey = selectedUser?.thread_id
    ? `thread-${selectedUser.thread_id}`
    : `user-${selectedUser?.other_participant?.user?.id}`;
    ...
    <div className="chat-page">
      ...
      <div className="chatSidebar">
        ...
        <div className="chatList">
          {showAllUsers ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '10px 20px', borderBottom: '1px solid #ddd' }}>
                <h4>All</h4> <h4>{chatUsersList.length}</h4>
              </div>
              {filteredAllUsers.map((user) => (
                <div
                  key={user.id}
            ...
            filteredUsers.map((chat) => (
              <div
                key={chat.latest_message.id}
                className={`chatItem ${
                  selectedUser?.latest_message.id === chat.latest_message.id ? 'selected' : ''
                }`}
                onClick={() => handleUserClick(chat)}
              >
                <div className="chatavatar">
                  <div className="avatar-placeholder">{chat.other_participant.user.name[0].toUpperCase()}</div>
                </div>
                <div style={{ width: '100%', overflow: 'hidden' }}>
                  <div className="chatName">{chat.other_participant.user.name}</div>
                  <div className="chatMessage">{chat.latest_message.body}</div>
                </div>
              </div>
            ))
          )}
      ...
      <div className="chatArea">
        ...
        <div className="chatMessages" id="chatMessages" key={chatKey}>
          {messagesData?.messages?.length ? (
            messagesData.messages.map((msg, index) => {
              ...
              return (
                <React.Fragment key={msg.id}>
                  {new Date(msg.created_at).toLocaleDateString() !== prevDate && (
                    <div className="dateLabel">
